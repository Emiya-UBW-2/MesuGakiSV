...
namespace Draw {
	...
	private:
		...
		int			m_BufferScreen{};				//バッファスクリーン
		int			m_MouseX{ InvalidID };			//バッファスクリーン上のマウスX座標
		int			m_MouseY{ InvalidID };			//バッファスクリーン上のマウスY座標
	...
	public://Setter
		...
		void		SetMousePosition(int MouseX, int MouseY) noexcept {
			this->m_MouseX = (MouseX * this->m_WindowWidth / DispWidth) + (this->m_WindowDrawWidth - this->m_WindowWidth) / 2;
			this->m_MouseY = (MouseY * this->m_WindowHeight / DispHeight) + (this->m_WindowDrawHeight - this->m_WindowHeight) / 2;
