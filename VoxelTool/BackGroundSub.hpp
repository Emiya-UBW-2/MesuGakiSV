...
namespace Algorithm {
	...
	// AABBと線分の当たり判定
	inline static bool		ColRayBox(const VECTOR& StartPos, VECTOR* EndPos, const VECTOR& AABBMinPos, const VECTOR& AABBMaxPos, VECTOR* Normal = nullptr) noexcept {
		VECTOR Vec = VSub(*EndPos, StartPos);
		// 交差判定
		...
		// 交差している
		if (EndPos) {
			*EndPos = VAdd(StartPos, VScale(Vec, t_min));
		}
		...
		if (Normal) {
			for (int loop = 0; loop < 3; ++loop) {
				if (std::abs(ret[loop] - min[loop]) < 0.00001f) {
					switch (loop) {
					case 0:
						*Normal = VGet(-1.f, 0.f, 0.f);
						break;
					case 1:
						*Normal = VGet(0.f, -1.f, 0.f);
						break;
					case 2:
						*Normal = VGet(0.f, 0.f, -1.f);
						break;
					default:
						break;
					}
	...
	inline static bool		CullingLine(int* MaxminT, int* MaxmaxT, float dTa, float dTb) noexcept {
		...
		if (!OnFront && !Onbehind) {
			if (dTa < 0.0f) {
				*MaxminT = Algorithm::Max(*MaxminT + static_cast<int>((*MaxmaxT - *MaxminT) * (dTa / (dTa - dTb))) - 1, *MaxminT);
			}
			else {
				*MaxmaxT = Algorithm::Min(*MaxminT + static_cast<int>((*MaxmaxT - *MaxminT) * (dTa / (dTa - dTb))) + 1, *MaxmaxT);
			}
		}
		return true;
	};
...
namespace BackGround {
	...
	class VoxelControl {
		...
		class CellsData {
			...
			public:
				const bool IsEmpty(void) const noexcept { return GetID() == s_EmptyBlick; }//セルが置かれていない
				const bool IsOcclusion(int id) const noexcept { return (this->m_FillInfo & (1 << id)) != 0; }//特定の辺の遮蔽を取得
				const bool CanDraw(void) const noexcept { return !IsEmpty() && (this->m_FillInfo != 0b111111); }// なにかセルが置かれていて、尚且つ6辺の何れかが見える状態
			};
		private:
		...
		public:
			const bool			isReferenceCell(void) const noexcept { return GetLODRate() == ReferenceCell; }// 自身が基準ボクセルタイルリストかどうか
			...
			const int8_t		isFill(int Xvoxel, int Yvoxel, int Zvoxel, int mul) const noexcept {
				...
				for (int xt = xMaxmin; xt < xMaxmin + mul; ++xt) {
					for (int yt = yMaxmin; yt < Algorithm::Min(yMaxmin + mul, this->All); ++yt) {
			...
			const VECTOR		GetWorldPos(const Algorithm::Vector3Int& VoxelPoint) const noexcept {
				return GetWorldPos(VoxelPoint.x, VoxelPoint.y, VoxelPoint.z);
			}
			// ボクセル座標から描画座標のXYZ各範囲に変換
			const VECTOR		GetWorldPosOffset(const Algorithm::Vector3Int& VoxelPoint, int Xofs, int Yofs, int Zofs) const noexcept {
				return GetWorldPos(VoxelPoint.x + Xofs, VoxelPoint.y + Yofs, VoxelPoint.z + Zofs);
			}
			// 指定の描画座標が入っている範囲のボクセル座標に変換
			const Algorithm::Vector3Int	GetVoxelPoint(const VECTOR& pos) const noexcept {
		...
		class VERTEX3DData {
			...
			std::array<size_t, 2>							m_32Num{ 0 };	// 登録した頂点数
		public:
		...
		public:
			...
			size_t		RegistPlane(void) noexcept {
				...
				if (this->m_32Num[this->m_Now] > this->m_32Size) {
					this->m_32Size = this->m_32Num[this->m_Now];
					// 両方ともリサイズする
					for (int loop = 0; loop < 2; ++loop) {
				...
				size_t Now = this->m_32Num[this->m_Now] - 1;
			...
			void		SetParamToPlane(int index, const VECTOR& pos, const VECTOR& norm, int Uoffset, int Voffset) noexcept {
		...
		class DrawThreadData {
			VECTOR				m_DrawCenterPos{};	// 描画時の中心座標
			VECTOR				m_CamVec{};			// 描画時の視点の向き
			VERTEX3DData		m_Vert32{};			// 描画先
			ThreadJobs			m_Jobs{};			// 生成用スレッド
		public:
			// 描画情報の取得
			const VECTOR& GetDrawCenterPos(void) const noexcept { return this->m_DrawCenterPos; }
			const VECTOR& GetCamVec(void) const noexcept { return this->m_CamVec; }
			// ジョブのセットアップ
			...
			// 描画情報の書き込み
			void		SetDrawInfo(const VECTOR& camPos, const VECTOR& camVec) noexcept {
	...
	private:
		...
		VECTOR							m_DrawCenterPos{};
		VECTOR							m_CamVec{};
		VECTOR							m_ShadowDrawCenterPos{};
		VECTOR							m_ShadowCamVec{};
	private:
		// 各方向に向いているポリゴンの追加
	...
	public:
		...
		// ボクセルとの線での当たり判定を取る
		int				CheckLine(const VECTOR& StartPos, VECTOR* EndPos, VECTOR* Normal = nullptr) const noexcept;
		// 対人を想定した壁判定を行う
		bool			CheckWall(const VECTOR& StartPos, VECTOR* EndPos, const VECTOR& AddCapsuleMin, const VECTOR& AddCapsuleMax, float Radius, const std::vector<MV1_HANDLE>& addonColObj) const noexcept;
		// ボクセルデータをロードする
		...
		inline float	GetDrawFarMax() const noexcept { return Algorithm::Min(Algorithm::Min(DrawMaxXPlus, DrawMaxZPlus), DrawMaxYPlus) * CellScale; }

		void			SetDrawInfo(const VECTOR& CenterPos, const VECTOR& CamVec) noexcept {
			this->m_DrawCenterPos = CenterPos;
			this->m_CamVec = CamVec;
		}
		void			SetShadowDrawInfo(const VECTOR& CenterPos, const VECTOR& CamVec) noexcept {
