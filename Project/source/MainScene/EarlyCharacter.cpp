...
namespace Charas {
	void HitBoxControl::Update(const BaseObject* ptr, float SizeRate) noexcept {
		...
		auto* Ptr = (EarlyCharacter*)ptr;
		size_t ID = 0;
		this->m_HitBox[ID].Update(Ptr->GetFrameWorldMat(CharaFrame::Head).pos(), 0.13f * SizeRate, HitType::Head); ++ID;
		this->m_HitBox[ID].Update((Ptr->GetFrameWorldMat(CharaFrame::Head).pos() + Ptr->GetFrameWorldMat(CharaFrame::Upper).pos()) / 2.0f, 0.16f * SizeRate, HitType::Body); ++ID;
